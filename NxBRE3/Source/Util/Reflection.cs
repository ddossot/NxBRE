namespace NxBRE.Util
{
    ...
    public abstract class Reflection
    {
        ...
        public static object CastValue(object valueObject, Type type)
        {
            ...
            return type.IsEnum ? Enum.Parse(type, valueObject.ToString(), false) : Convert.ChangeType(valueObject, type, CULTURE_INFO);
        }
        ...
        public static void CastToStrongType(ObjectPair pair)
        {
            if ((pair.First.GetType().GetInterface("System.IConvertible", false) == null) ||
                (pair.Second.GetType().GetInterface("System.IConvertible", false) == null)) return;
        ...
        public static object ClassNew(string type, object[] argValues)
        {
            ...
            var toInstantiate = GetRuntimeType(type);
            var ci = toInstantiate.GetConstructor(GetArgumentTypes(argValues));
        ...
        private static object CallMethodWithByRefParametersOrThrow(Type type, object target, string methodName, object[] args, TargetException te)
        {
            ...
            foreach (var mi in mis)
            {
                ...
                if ((!mi.Name.Equals(methodName)) || (parameters.Length != args.Length)) continue;
                var allMatch = true;
        ...
        {
            var runtimeType = Type.GetType(type, true);
        ...
        {
            var argumentTypes = new ArrayList();

            foreach (var t in arguments)
            {
                argumentTypes.Add(t?.GetType() ?? typeof (object));
            }
