    ...
    public sealed class IEImpl : IInferenceEngine
    {
        ...
        public void SaveRuleBase(IRuleBaseAdapter adapter)
        {
            ...
            using (adapter)
            {
                ...
                //queries
                IList<Query> queries = QB.ToList();
                adapter.Queries = queries;

                // implications
                IList<Implication> implications = IB.ToList();
                adapter.Implications = implications;

                // build a collection of in-memory facts
                IList<Fact> factsInWorkingMemory = WM.FB.ToList();

                var baseAdapter = adapter as IExtendedRuleBaseAdapter;
                if (baseAdapter != null)
                {
                    // equivalents & integrity queries, assertions and retractions, if supported
                    var extendedAdapter = baseAdapter;
        ...
        public void LoadFacts(IFactBaseAdapter adapter)
        {
            ...
            using (adapter)
            {
                ...
                // load facts
                var initialFactsCount = WM.FB.Count;
                foreach (var fact in adapter.Facts) Assert(fact);
        ...
        {
            var fact = GetFact(factLabel);

            return fact != null && WM.FB.Retract(fact);
        }
        ...
        {
            var currentFact = GetFact(currentFactLabel);

            return currentFact != null && WM.FB.Modify(currentFact, newFact);
        }
        ...
        public IList<string> QueryLabels
        {
            ...
            {
                CheckInitialized();
                var result = new string[QueriesCount];
                var i = 0;

                foreach (var query in QB)
                {
        ...
        private void ProcessPerfomatives()
        {
            ...
            if (performativeAssertions == null) return;
            foreach (var fact in performativeAssertions) Assert(fact);
        }
        ...
        private void ProcessConnectives(IDictionary businessObjects)
        {
            ...
            else switch (Binder.BindingType)
                {
                    case BindingTypes.BeforeAfter:
                        {
                            var iniTime = DateTime.Now.Ticks;
                            ...
                            var binderIterate = true;
                            var positiveImplications = new ArrayList();
        ...
        private void InferUntilNoNewFact()
        {
            ...
            while (iterate)
            {
                ...
                {
                    var firedImplication = agenda.NextToExecute;
                    ...
                    if (firedImplication == null) continue;
                    WM.FB.ModifiedFlag = false;

                    var resultsCount = RunImplication(firedImplication);
            ...
            foreach (var integrityQuery in from integrityQuery in IntegrityQueries let qrs = RunQuery(integrityQuery) where qrs.Count == 0 select integrityQuery)
            {
        ...
        {
            var implicationResultsCount = 0;

            var processResults = WM.FB.ProcessAtomGroup(implication.AtomGroup);
                  ...
                  || (implication.Action == ImplicationAction.Retract))
            {
                ...
                {
                    var deductedFact = BuildFact(implication.Deduction, processResult);

                    if (deductedFact == null) continue;
                    implicationResultsCount++;

                    if (implication.Action == ImplicationAction.Retract)
                    {
                        // retracting implication factbase action
                        var result = WM.FB.Retract(deductedFact);
                    ...
                    else
                    {
                        ...
                        var result = WM.FB.Assert(deductedFact);
            ...
            else if (implication.Action == ImplicationAction.Modify)
            {
                foreach (var processResult in processResults)
                {
                    ...
                    var modificationTargetLookup = FactBase.BuildQueryFromDeduction(implication.Deduction, processResult);
                    ...
                    foreach (var factToModify in FactBase.ExtractAllFacts(WM.FB.ProcessAtomGroup(new AtomGroup(AtomGroup.LogicalOperator.And, modificationTargetLookup))))
                    {
                        ...
                        var deductedFact = BuildFact(implication.Deduction,
                        ...
                        if ((deductedFact == null) || (factToModify.Equals(deductedFact))) continue;
                        implicationResultsCount++;
                        var result = WM.FB.Modify(factToModify, deductedFact);
        ...
        {
            var factBuilder = FactBase.Populate(templateAtom, processResult, true);
