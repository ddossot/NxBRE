...
<xsl:stylesheet version="1.0" exclude-result-prefixes="vdx" xmlns="http://www.ruleml.org/0.86/xsd" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:vdx="http://schemas.microsoft.com/visio/2003/core">
		...
		<_rlab>
			<ind>
				...
				<xsl:call-template name="property.value">
					<xsl:with-param name="property-name">Label</xsl:with-param>
				</xsl:call-template>
				
				<xsl:text>;priority:</xsl:text>
				<xsl:call-template name="property.value">
					<xsl:with-param name="property-name">Priority</xsl:with-param>
				</xsl:call-template>
				
				<xsl:text>;mutex:</xsl:text>
				<xsl:call-template name="property.value">
					<xsl:with-param name="property-name">Mutex</xsl:with-param>
				</xsl:call-template>
				
				<xsl:text>;precondition:</xsl:text>
				<xsl:call-template name="property.value">
					<xsl:with-param name="property-name">Precondition</xsl:with-param>
				</xsl:call-template>
	...
	<xsl:template name="property.value">
		<xsl:param name="property-name"/>
		<xsl:variable name="MasterID" select="@Master"/>
		
		<xsl:choose>
			<xsl:when test="not(vdx:Prop[vdx:Label=$property-name]/vdx:Value)">
				<xsl:value-of select="/vdx:VisioDocument/vdx:Masters/vdx:Master[@ID=$MasterID]//vdx:Prop[vdx:Label=$property-name]/vdx:Value/text()"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="vdx:Prop[vdx:Label=$property-name]/vdx:Value/text()"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
</xsl:stylesheet>
